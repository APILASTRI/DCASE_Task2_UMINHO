...
def get_model(inputDim, name_for_model=""):
    ...
    """
    # x (100*100*100*100*64*100*100*100*100) activation elu, optimizer adam base x spectogram = 2' --> best para o
    # slider!
    ...
    for f in foldersForChallenge:
        callbacks_listv1 = [
            keras.callbacks.EarlyStopping(
                ...
                monitor='val_loss',
                mode='min',
                filepath=path + "/modelBest" + "_" + f + ".H5",
                save_best_only=True,
                verbose=1
            )]
        files_Read = []
        print("Machine Processing", f)
        os.chdir(path + "/" + f + "/train/")
        ...
        print("pAUC Average", numpy.mean(data_csv["pAUC"]))
